...
#5.2编辑距离
先给出编辑距离的定义：设A和B是2个字符串，要用最少的字符操作将字符串A转换为字符串
B。这里所说的字符操作包括：
    (1)删除一个字符(delete)；
    (2)插入一个字符(insert)；
    (3)将一个字符改为另一个字符(substitute)。
编辑距离具有下面几个性质：
<1>两个字符串的最小编辑距离至少是两个字符串的长度差；
<2>两个字符串的最大编辑距离至多是两字符串中较长字符串的长度；
<3>两个字符串的编辑距离是零的充要条件是两个字符串相同；
<4>如果两个字符串等长，编辑距离的上限是汉明距离(Hamming distance)；
<5>编辑距离满足三角不等式，即d(a, c) ≤ d(a, b) + d(b, c)；
<6>如果两个字符串有相同的前缀或后缀，则去掉相同的前缀或后缀对编辑距离没有影响，
其他位置不能随意删除。
...
公式：
    ...
    我们解释一下上面的公式，前两行是初始化，分别表示字符串b和a为空时对应的编辑距
	离计算。当字符串b为空时，我们将a变为b只需要不停地删除字符即可，反之则不停地
	添加字符。初始化完矩阵d的最初一行和一列之后我们就可以按照第三行公式去计算矩
	阵的剩余元素。第(i,j)个元素在计算的时候依赖于和它相邻的三个位置(i-1,j-1)、
	(i,j-1)和(i-1,j)。公式的整体结构和LCS，DTW都非常像，唯一存在的难点就是到底哪
	一个位置对应删除操作，哪一个位置对应插入操作。由公式可以看出，(i-1,j)对应删
	除操作，(i,j-1)对应插入操作。可以这样理解，现在耗费了di-1,j步操作将字符串
	a(1,i-1)转换成了b(1,j)，则在将a(1,i)转换成b(1,j)时，我们可以直接删掉字符
	a(i)，问题变成a(1,i-1)转换成b(1,j)，从而dij就等于di-1,j+1。同理，现在耗费了
	di,j-1步操作将字符串a(1,i)转换成了b(1,j-1)，则在将a(1,i)转换成b(1,j)时，我们
	可以将b(j)添加到a(1,i)末尾（此时a(1,i)已转换成b(1,j-1)）构成b(1,j)。
...
#5.4交替字符串
输入三个字符串s1, s2, t; 判断t是否由前两个s1和s2交错而成且不改变s1和s2中各个字符
原有的相对顺序。
...
boolean IsIterLeave(char[] s1, char[] s2, char[] t)
{
    ...
    for (int i = 0; i < m + 1; i++)
    {
        ...
        {
            if (dp[i][j] 
                    || (i - 1 >= 0 && dp[i - 1][j] == true && s1[i - 1] == t[i + 
					j - 1]) 
                    || (j - 1 >= 0 && dp[i][j - 1] == true && s2[j - 1] == t[i + 
					j - 1]))
            {
                dp[i][j] = true;
            } else
